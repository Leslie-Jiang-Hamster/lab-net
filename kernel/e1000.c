...
e1000_transmit(struct mbuf *m)
{
  ...
  uint32 next_packet_index = regs[E1000_TDT];
  struct tx_desc next_packet_descriptor = tx_ring[next_packet_index];
  ...
  if (tx_mbufs[next_packet_index] != (void *)0)
    mbuffree(tx_mbufs[next_packet_index]);
  ...
  tx_ring[next_packet_index].addr = (uint64)m->head;
  tx_ring[next_packet_index].length = (uint64)m->len;
  tx_ring[next_packet_index].cmd = E1000_TXD_CMD_EOP | E1000_TXD_CMD_RS;
  tx_mbufs[next_packet_index] = m;
...
e1000_recv(void)
{
  ...
  struct rx_desc next_packet_descriptor = rx_ring[next_packet_index];
  ...
  // If not, stop.
  if (!(E1000_RXD_STAT_DD & next_packet_descriptor.status))
    return;
  ...
  rx_mbufs[next_packet_index]->len = next_packet_descriptor.length;
  ...
  rx_mbufs[next_packet_index] = mbufalloc(MBUF_SIZE);
